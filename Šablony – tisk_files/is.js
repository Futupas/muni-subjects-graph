...
is.Define.class('TableSort', function(namespace, $) {
	...
	var default_config = {
		...
		headers: {},
		textSorter: {},
		selectorHeaders: '> thead td:not(.ne_sort), > thead th:not(.ne_sort)',
	};
	...
	$.extend(TableSort.prototype, {
		reinit_cols : function () {
			...
			self.$tabulka.find('> thead > tr').each(function () {
			    $(this).find('th,td').each(function (index, el) {
				var razeni, jazyk;
				var $el = $(el);
...
is.Define.module('Forms', function(namespace) {
	...
	Forms.set_custom_validator = function (name, err_msg_lid, validator) {
		var callback;
		validators = validators || window.Foundation.Abide.defaults.validators;
